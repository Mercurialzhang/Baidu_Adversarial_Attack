...
def tensor2img_floor(tensor, raw):
    img_mean = np.array(mean).reshape((3, 1, 1))
    img_std = np.array(std).reshape((3, 1, 1))
    ...
    img = np.sign(img - raw) * np.floor(np.abs(img - raw)) + raw
    img = np.clip(img, 0, 255)
...
def calc_mse(org_img, adv_img):
    diff = org_img.reshape((-1, 3)).astype('float32') - adv_img.reshape((-1, 3)).astype('float32')
